...
from training.latent_learner import DirectionInterpolator, PCA
...
def training_loop_tl(
    ...
    if rank == 0:
        ...
        c = torch.empty([batch_gpu, G.c_dim], device=device)
        img = misc.print_module_summary(G, [z, c])
        misc.print_module_summary(T, [img])
        #misc.print_module_summary(D, [img, c])
    ...
    while True:
        ...
        with torch.autograd.profiler.record_function('data_fetch'):
            ...
            #phase_real_img = (phase_real_img.to(device).to(torch.float32) / 127.5 - 1).split(batch_gpu)
            #phase_real_c = phase_real_c.to(device).split(batch_gpu)
            phase_real_img, phase_real_c = None, None 
            all_gen_z = torch.randn([len(phases) * batch_size, G.z_dim], device=device)
            all_gen_z = [phase_gen_z.split(batch_gpu) for phase_gen_z in all_gen_z.split(batch_size)]
            #all_gen_c = [training_set.get_label(np.random.randint(len(training_set))) for _ in range(len(phases) * batch_size)]
        ...
        if (rank == 0) and (image_snapshot_ticks is not None) and (done or cur_tick % image_snapshot_ticks == 0):
            ...
            with torch.no_grad():
                ...
                ws = torch.cat([G_ema.mapping(z=z, c=c) for z, c in zip(grid_z, grid_c)])
                psi = torch.rand((ws.shape[0], 1)).to(device)
        ...
        """
        Self-supervision does not rely on real data
        """
        """
        if (snapshot_data is not None) and (len(metrics) > 0):
            if rank == 0:
