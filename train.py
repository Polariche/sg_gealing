...
def subprocess_fn(rank, c, temp_dir):
    ...
    training_loop.training_loop_tl(rank=rank, **c)
...
def launch_training(c, desc, outdir, dry_run):
    ...
    if "training_set_kwargs" in c.keys():
...
@click.option('--tick',         help='How often to print progress', metavar='KIMG',             type=click.FloatRange(min=0), default=1, show_default=True)
...
def main(**kwargs):
    ...
    c.T_opt_kwargs = dnnlib.EasyDict(class_name='torch.optim.Adam', betas=[0,0.99], eps=1e-8)
    c.loss_kwargs = dnnlib.EasyDict(class_name='training.loss.TransformerSiameseLoss')
    ...
    c.T_opt_kwargs.lr = (0.002 if opts.cfg == 'stylegan2' else 0.0025) if opts.glr is None else opts.glr
    c.metrics = opts.metrics
    c.total_kimg = opts.kimg
    c.kimg_per_tick = opts.tick
    c.image_snapshot_ticks =  opts.snap
    c.network_snapshot_ticks = opts.snap * 100
    c.random_seed = opts.seed #c.training_set_kwargs.random_seed = opts.seed
    ...
    if opts.fp32:
        c.G_kwargs.num_fp16_res = 0 #c.D_kwargs.num_fp16_res = 0
        c.G_kwargs.conv_clamp = None#c.D_kwargs.conv_clamp = None
    if opts.nobench:
